            ...
            case 'g': case 'G':
                ...
                if (size == 'L') {
                    ...
                    if (_INST_Main_PointerAnalysis && _INST_Main_PointerAnalysis->isSVPtr(a_ptr)) {
                        b_ldbl = ((FPSV*)a_ptr)->getValue(C99_LongDouble).data.ldbl;
                        is_ptr = true;
                    }
                    if (_INST_Main_InplaceAnalysis && _INST_Main_InplaceAnalysis->isReplaced(a_ptr)) {
                        b_ldbl = (double)(*(float*)(&a_ptr));
                        is_ptr = true;
                    }
                    ...
                    if (_INST_Main_PointerAnalysis && _INST_Main_PointerAnalysis->isSVPtr(a_ptr)) {
                        b_dbl = ((FPSV*)a_ptr)->getValue(IEEE_Double).data.dbl;
                        is_ptr = true;
                    }

                    if (_INST_Main_InplaceAnalysis && _INST_Main_InplaceAnalysis->isReplaced(a_ptr)) {
                        b_dbl = (double)(*(float*)(&a_ptr));
                        is_ptr = true;
                    }
